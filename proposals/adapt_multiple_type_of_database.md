...
![notary.png](images/multidb/notary.png)

make/photon/prepare/templates/notary/server-config.mysql.json.jinja
```json
{
	"server": {
		"http_addr": ":4443"
	...
	"storage": {
		"backend": "mysql",
		"db_url": "{{notary_server_db_username}}:{{notary_server_db_password}}@tcp({{notary_server_db_host}}:{{notary_server_db_port}})/{{notary_server_db_name}}?parseTime=True"
    },
    "auth": {
        "type": "token",
        "options": {
